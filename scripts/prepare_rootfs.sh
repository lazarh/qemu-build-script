...
# Use debootstrap + qemu-user-static path (avoid qemu-debootstrap which may map arch unexpectedly)

echo "Using debootstrap + qemu-user-static to build Debian $DEBIAN_DIST ($ARCH) at $ROOTFS_DIR from mirror $MIRROR"
sudo apt-get update
...
for m in "${MIRRORS[@]}"; do
  ...
  mkdir -p "$ROOTFS_DIR"
  if sudo debootstrap --arch="$ARCH" --variant=minbase --foreign "$DEBIAN_DIST" "$ROOTFS_DIR" "$m"; then
